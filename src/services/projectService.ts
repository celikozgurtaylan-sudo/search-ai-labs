...
export const projectService = {
  ...
  async getUserProjects(includeArchived: boolean = false): Promise<Project[]> {
    const { data: { user } } = await supabase.auth.getUser();
    if (!user) throw new Error('User must be authenticated to view projects');
  ...
  async getArchivedProjects(): Promise<Project[]> {
    ...
    if (error) throw new Error(`Failed to fetch archived projects: ${error.message}`);
    return data || [];
  },

  async getProject(id: string): Promise<Project | null> {
    const { data: { user } } = await supabase.auth.getUser();
    if (!user) throw new Error('User must be authenticated to view project');
  ...
  async updateProject(id: string, updates: Partial<Omit<Project, 'id' | 'user_id' | 'created_at' | 'updated_at'>>): Promise<Project> {
    const { data: { user } } = await supabase.auth.getUser();
    if (!user) throw new Error('User must be authenticated to update projects');
  ...
  async archiveProject(id: string): Promise<void> {
    const { data: { user } } = await supabase.auth.getUser();
    if (!user) throw new Error('User must be authenticated to archive projects');
  ...
  async unarchiveProject(id: string): Promise<void> {
    const { data: { user } } = await supabase.auth.getUser();
    if (!user) throw new Error('User must be authenticated to unarchive projects');
  ...
  async deleteProject(id: string): Promise<void> {
    const { data: { user } } = await supabase.auth.getUser();
    if (!user) throw new Error('User must be authenticated to delete projects');
